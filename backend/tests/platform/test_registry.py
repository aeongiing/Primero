...
@pytest.mark.parametrize("platform", ["karrot", "fruits", "charan", "junggonara"])
...
def test_bunjang_is_api_adapter_no_browser_needed():
    adapter = get_adapter("bunjang")
    assert adapter.platform == "bunjang"
    assert isinstance(adapter, PlatformAdapter)
